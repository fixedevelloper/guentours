...
function MultiCityFlightsPageContent() {
  ...
  const setStoreMultiCityParams = useFlightStore((state) => state.setMultiCitySearchParams);
  const setStoreMultiCityResults = useFlightStore((state) => state.setMultiCityResults);
  const setStoreLoading = useFlightStore((state) => state.setLoading);
  const setStoreError = useFlightStore((state) => state.setError);

  useEffect(() => {
    if (params) {
      setStoreMultiCityParams(params);
    }
  }, [params, setStoreMultiCityParams]);
  ...
  useEffect(() => {
    if (query.isError) {
      setStoreError(t("noResults") ?? "Impossible de charger les itinéraires.");
    }
  }, [query.isError, setStoreError, t]);

  useEffect(() => {
    if (query.data) {
      setStoreMultiCityResults(query.data);
    }
  }, [query.data, setStoreMultiCityResults]);
  // --- Fin synchronisation ---
