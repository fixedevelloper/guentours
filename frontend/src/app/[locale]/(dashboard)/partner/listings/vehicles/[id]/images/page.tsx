...
import { VehicleImageResponse } from "@/lib/api/types";
...
function toVehicleImageItem(image: VehicleImageResponse): VehicleImageItem {
    return {
        id: image.id,
        url: image.url,
...
export default function VehicleImagesPage({ params }: PageProps) {
    // Dépaquetage du paramètre asynchrone Next.js 15
    const { id } = use(params);
    const t = useTranslations("VehicleImages");
    ...
    useEffect(() => {
        ...
        getVehicle(partnerId, id)
            .then((vehicle) => {
                if (cancelled) return;
                setImages(vehicle.images.map(toVehicleImageItem));
            })
            .catch((error) => {
                console.error("Erreur lors du chargement des images du véhicule:", error);
                if (!cancelled) setLoadError("Impossible de charger les images de ce véhicule.");
            })
            .finally(() => {
    ...
    // Sélection d'image depuis la modale (bibliothèque personnelle) : ajoutée à la galerie du véhicule
    ...
    const handleSelectImage = async (selectedImage: { url: string; name: string }) => {
        ...
        try {
            const vehicle = existing
                ? await setPrimaryVehicleImage(partnerId, id, existing.id)
                : await addVehicleImage(partnerId, id, {
    ...
    const handleSetPrimary = async (imageId: string) => {
        ...
        try {
            const vehicle = await setPrimaryVehicleImage(partnerId, id, imageId);
            setImages(vehicle.images.map(toVehicleImageItem));
        } catch (error) {
    ...
    const handleDelete = async (imageId: string) => {
        ...
        try {
            await deleteVehicleImage(partnerId, id, imageId);
        } catch (error) {
    ...
    return (
        <div className="space-y-6">
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <h1 className="text-lg font-black tracking-tight sm:text-xl">
                                {t("title") ?? "Galerie du véhicule"}
                            </h1>
                        </div>
                        <p className="text-xs font-semibold text-muted-foreground">
                            ID Véhicule : <span className="font-mono text-foreground">{id}</span>
                        </p>
                    </div>
                </div>
            ...
            <Card className="border-2 border-dashed border-border/60 hover:border-primary/50 transition-all rounded-2xl bg-card/50">
                <CardContent className="flex flex-col items-center justify-center p-8 text-center">
                    ...
                    <h3 className="text-sm font-bold text-foreground">
                        {t("uploadTitle") ?? "Ajouter des visuels pour ce véhicule"}
                    </h3>
                    <p className="text-xs text-muted-foreground max-w-md mt-1 mb-4 font-medium">
                        Mettez en valeur l&apos;extérieur, l&apos;intérieur et les équipements du véhicule.
                    </p>
                    <Button
                        variant="secondary"
                        size="sm"
                ...
                <div className="space-y-3">
                    <div className="flex items-center justify-between">
                        <h2 className="text-xs font-black uppercase tracking-wider text-muted-foreground">
                            {t("sectionTitle") ?? "Photos du véhicule"}
                        </h2>
                            ...
                            >
                                ...
                                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2 backdrop-blur-[2px]">
                                    <Button
                                        ...
                                        title="Définir comme couverture du véhicule"
                                    >
                                        <Star
                                            className={cn(
                                                "size-4",
            ...
            <ImageSelectModal
                ...
                title="Sélectionner la couverture du véhicule"
            />
        </div>
    );
}
